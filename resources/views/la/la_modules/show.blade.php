...
<div id="page-content" class="profile2">
	...
	@endif
		<div class="col-md-4">
			<div class="row">
				...
				<div class="col-md-9">
					<a @ajaxload class="text-white" href="{{ url(config('laraadmin.adminRoute') . '/'.$module->name_db) }}"><h4 data-toggle="tooltip" data-placement="left" title="Open {{ $module->model }} Module" class="name">{{ $module->label }}</h4></a>
					<div class="row stats">
						<div class="col-md-12">{{ LAModule::itemCount($module->name) }} Items</div>
					</div>
	...
	<ul id="module-tabs" data-toggle="ajax-tab" class="nav nav-tabs profile" role="tablist">
		<li class=""><a href="{{ url(config('laraadmin.adminRoute') . '/la_modules') }}" data-toggle="tooltip" data-placement="right" title="Back to Modules"> <i class="fa fa-chevron-left"></i>&nbsp;</a></li>
	...
	<div class="tab-content">
						...
						<tbody>														
							@foreach ($module->fields as $field)
								<tr>
									...
									<td>
										<form class="listing_view_cal" action="{{ url(config('laraadmin.adminRoute') . '/la_module_fields_listing_show') }}">
									...
									<td style="min-width:60px;">
										<a @ajaxload href="{{ url(config('laraadmin.adminRoute') . '/la_module_fields/'.$field['id'].'/edit') }}" class="btn btn-edit-field btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;" id="edit_{{ $field['colname'] }}"><i class="fa fa-edit"></i></a>
										<form action ="{{ url(config('laraadmin.adminRoute') . '/la_module_fields/'.$field['id'].'/delete') }}", method = 'delete' style ='display:inline'>
											<input name="_method" type="hidden" value="DELETE">
											<button class="btn btn-danger btn-xs" type="submit"><i class="fa fa-trash"></i></button>
										</form>
										<!-- <a href="{{ url(config('laraadmin.adminRoute') . '/la_module_fields/'.$field['id'].'/delete') }}" class="btn btn-edit-field btn-danger btn-xs" style="display:inline;padding:2px 5px 3px 5px;" id="delete_{{ $field['colname'] }}"><i class="fa fa-trash"></i></a> -->
										@if($field['colname'] != $module->view_col)
											<a @ajaxload href="{{ url(config('laraadmin.adminRoute') . '/la_modules/'.$module->id.'/set_view_col/'.$field['colname']) }}" class="btn btn-edit-field btn-success btn-xs" style="display:inline;padding:2px 5px 3px 5px;" id="view_col_{{ $field['colname'] }}"><i class="fa fa-eye"></i></a>
										@endif
									</td>
								</tr>
							@endforeach
		...
		<div role="tabpanel" class="tab-pane fade in p20 bg-white" id="tab-access">
			...
			<form action="{{ url(config('laraadmin.adminRoute') . '/la_modules/save_role_module_permissions/'.$module->id) }}" method="post">
                                    ...
                                    <td colspan=6>
                                        ...
                                        <div class="col-md-2 col-lg-2 pb10 pt10 all_field_div">
											@php
												...
												$two = 0;
												foreach($module->fields as $field) {
													if($role->fields[$field['id']]['access'] == "2") {
														$two++;
													} elseif($role->fields[$field['id']]['access'] == "1") {
														$one++;
													}
												}
												if(count($module->fields) == $two) {
													$value_access = 2;
												} elseif(count($module->fields) == $one) {
													$value_access = 1;
												}
											@endphp
											<input type="text" name="{{ $module->name }}_{{ $role->id }}" class="form-control slider all_field" role-id="{{ $role->id }}" value="{{ $value_access }}" data-slider-value="{{ $value_access }}" data-slider-min="0" data-slider-max="2" data-slider-step="1" data-slider-orientation="horizontal">
                                        </div>
                                        <div class="col-md-5 col-lg-5"></div>
                                    </td>
                                </tr>
								@foreach (array_chunk($module->fields, 3, true) as $fields)
									<tr>
										@foreach ($fields as $field)
											<td>
												<div class="col-md-3">
													<input type="text" name="{{ $field['colname'] }}_{{ $role->id }}" value="{{ $role->fields[$field['id']]['access'] }}" data-slider-value="{{ $role->fields[$field['id']]['access'] }}" class="slider form-control role_{{ $role->id }}" data-slider-min="0" data-slider-max="2" data-slider-step="1" data-slider-orientation="horizontal"  data-slider-id="{{ $field['colname'] }}_{{ $role->id }}">
												</div>
												{{ $field['label'] }}
											</td>
										@endforeach
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-footer">
				{{ Form::open(['route' => [config('laraadmin.adminRoute') . '.la_modules.destroy', 0], 'id' => 'module_del_form', 'method' => 'delete', 'style'=>'display:inline']) }}
...
<div class="modal fade" id="AddFieldModal" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			{!! Form::open(['route' => config('laraadmin.adminRoute') . '.la_module_fields.store', 'id' => 'field-form']) !!}
				...
				<div class="box-body">
					...
					<div class="form-group">
						...
						foreach($columns as $col) {
							...
							$field = LAModuleField::where('colname', $col)->where('module', $module->id)->first();
                    ...
                    <div class="form-group">
						...
						{{ Form::text("comment", null, ['class'=>'form-control', 'placeholder'=>'Field Comment', 'data-rule-minlength' => 0, 'data-rule-maxlength'=>180]) }}
					</div>
...
.all_field_div .slider-track, .all_field_div .slider-selection {
...
$(function () {
	...
	$('.delete_module').on("click", function () {
		...
		$.ajax({
			url: "{{ url(config('laraadmin.adminRoute') . '/la_modules/get_module_files/') }}/" + module_id,
			type:"POST",
			beforeSend: function() {
			...
			success: function(data) {
				...
				for ($i = 0; $i < files.length; $i++) { 
					filesList += "<li>" + files[$i]['name'] + "<span class='pull-right " + files[$i]['class'] + "'>" + files[$i]['perms'] + "</span>" + "</li>";
				}
				filesList += "</ul>";
	...
	$("select[name='field_type']").on("change", function() {
		showValuesSection();
        if($(this).find('option:selected').html() == "JSON") {
            alert("Field Type 'JSON' supported on ^MySQL-5.7.8 and ^MariaDB-10.2. Please check JSON support with your Database.");
        }
	});
	showValuesSection();
	...
	$("#sortable_module_fields").sortable({
		update: function(event, ui) {
			...
			$.ajax({
				url: "{{ url(config('laraadmin.adminRoute') . '/la_modules/save_module_field_sort') }}/"+{{ $module->id }},
	...
	$("#generate_migr").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url(config('laraadmin.adminRoute') . '/la_modules/generate_migr') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
	...
	$("#generate_update").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url(config('laraadmin.adminRoute') . '/la_modules/generate_update') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
				$fa.removeClass("fa-refresh");
				$fa.removeClass("fa-spin");
	...
	$("#generate_migr_crud").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url(config('laraadmin.adminRoute') . '/la_modules/generate_migr_crud') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
				$fa.removeClass("fa-refresh");
				$fa.removeClass("fa-spin");
    ...
    */
    ...
    $(".all_field").on("change", function() {
        ...
        console.log("role_id: " + role_id + " - all_field_value:" + all_field_value);

        $('.role_'+role_id).slider('setValue', parseInt(all_field_value));

		switch (all_field_value) {
			case '0':
				$('.role_'+role_id).parent().find('.slider').removeClass("orange");
				$('.role_'+role_id).parent().find('.slider').removeClass("green");
				$('.role_'+role_id).parent().find('.slider').addClass("gray");
				break;
			case '1':
				$('.role_'+role_id).parent().find('.slider').removeClass("gray");
				$('.role_'+role_id).parent().find('.slider').removeClass("green");
				$('.role_'+role_id).parent().find('.slider').addClass("orange");
				break;
			case '2':
				$('.role_'+role_id).parent().find('.slider').removeClass("gray");
				$('.role_'+role_id).parent().find('.slider').removeClass("orange");
				$('.role_'+role_id).parent().find('.slider').addClass("green");
				break;
		}
		if(parseInt(all_field_value) != 0) {
			$("#module_view_" + role_id).prop('checked', true);
		} else {
			$("#module_view_" + role_id).prop('checked', false);
		}
	});
    ...
    $(".listing_view_cal .Switch.Ajax").click(function() {
        ...
        $.ajax({
			type: "POST",
			url : "{{ url(config('laraadmin.adminRoute') . '/la_module_fields_listing_show') }}",
			data : {
				_token: '{{ csrf_token() }}',
    ...
    }), $(".listing_view_cal .Switch.Ajax").each(function() {
